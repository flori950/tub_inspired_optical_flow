...
max_index = 115; % set the max index of csv files
num_clusters_in = 2; % set number of initial clusters
step_mu = 0.00001; % set gradient ascent rate 0.00001 fine but slow
...
a = load('Hands_sequence.mat','davis');
b = a.davis.dvs;
c = [b.x,b.y,b.t*10^6];
...
for i = 1:size_P(2)
    figure(30+i);
    scatter(evt_vec(:,1),-evt_vec(:,2), 1, P(:,i));
    title(strcat('Cluster ',num2str(i)));
    colorbar;colormap(jet(3));
end
end

function plotIWE(weightedIWE, index)
size_weightedIWE = size(weightedIWE);
disp(size_weightedIWE);
disp(length(size_weightedIWE));
