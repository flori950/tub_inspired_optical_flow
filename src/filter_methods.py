...
def gabor_filter_even(x, y, delta, theta, f0x, f0y):
    ...
    pi = math.pi
    Gabor_k = 1./(2*pi*delta**2)*np.exp(-1./(2*delta**2)*((x_hat-params.x0)
...
def filter_mono(t):
    value = params.mono_wm1*np.exp(-(t-params.mono_mium1())**2/(2*params.mono_sigmam1(
    )**2))+params.mono_wm2*np.exp(-(t-params.mono_mium2())**2/(2*params.mono_sigmam2()**2))
    return value


def filter_bi(t):
    value = params.bi_wm1*np.exp(-(t-params.bi_mium1())**2/(2*params.bi_sigmam1(
