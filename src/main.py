...
np.save(params.kernel_route + '/space_time_kernel1.npy', space_time_kernel_1) # adjust folder if needed
np.save(params.kernel_route + '/space_time_kernel2.npy', space_time_kernel_2) # adjust folder if needed
np.save(params.kernel_route + '/space_time_kernel_combined.npy', space_time_kernel_full) # adjust folder if needed
...
grid_vox = np.zeros((params.band_height,params.band_width,num_bins_tev),dtype=np.float32)
# t_begin = time.time()
with Timer("synchronous solution computing..."):
    for ie in range(0,params.num_events):
        ...
        if ((params.offset_height <= y) and (y < params.offset_height + params.band_height)
            ...
            exponent_space = -((xv-x)**2 + (yv-y)**2) / (2*(params.sigma_xy**2))  # units: pixel^2 / pixel^2
            exponent_time = -((tv-t_bin_coord)**2) / (2*(params.sigma_t**2))      # units: bin^2 / bin^2
...
for i in range(len(filters)):
    filename = params.kernel_route+'/kernel' + str(i) + '.npy' # adjust folder if needed
...
X,Y=np.meshgrid(np.arange(0,params.band_width),np.arange(0,params.band_height))
